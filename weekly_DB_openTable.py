...
for i in range(0, len(data_index)):
    r_p = '마일스톤' + ': ' + table_version.loc[data_index[i]]['name'] + '&sbquo;'
...
for i in range(0, len(data_index)):
    r_t = '<tr><td>' + '마일스톤' + '</td>' + \
    '<td>' + table_version.loc[data_index[i]]['name'] + '</td>' +\
...
tableRow2 = ''
for i in range(0, len(data_index)):
    r_t2 = '<tr><td style="text-align: center;" colspan="1"><a href="https://tcs.telechips.com:8443/projects/TPD/versions/' + \
    table_version.loc[data_index[i]]['versionId'] + '">' + \
    table_version.loc[data_index[i]]['name'] + '</a></td><td style="text-align: center;" colspan="1">' + \
    ...
    '<a style="text-align: center;" href="https://tcs.telechips.com:8443/issues/?jql=status%20not%20in(Closed%2C%20Resolved)%20and%20duedate%20%3C%20now()%20and%20fixVersion%20%3D%20' + \
...
wiki_data = '<h3><span style="color: rgb(0,0,255);"><strong>●&nbsp;</strong>\
...
<ac:parameter ac:name="tfc-height">180</ac:parameter>\
