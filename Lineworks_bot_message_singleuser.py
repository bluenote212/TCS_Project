import json, requests
import sqlite3
import pandas as pd

#user_data를 DB에서 가져와서 리스트로 변환
con = sqlite3.connect('C:/Users/B180093/database/tcs.db')
user_data = pd.read_sql("SELECT * FROM userData  WHERE = '신호찬 (Chance H Shin)'", con) # WHERE 조건 확인
con.close()
user_data = user_data.values.tolist()

headers = {
    'Content-Type': 'application/json; charset=UTF-8',
    'consumerKey': 'IuAF6mCrW4FiTxWF6bmm',
    'Authorization': 'Bearer '
}


url = 'https://apis.worksmobile.com/r/kr1llsnPeSqSR/message/v1/bot/1809717/message/push' #1:1 메시지 Request URL

for i in range(0, len(user_data)):
    body = {
        'botNo': '1809717',
        'accountId': user_data[i][3],
        'content': {
            'type': 'text',
            'text': '안녕하세요 R&D Bot 입니다.\n2021년 2월 연구소 Worklog를 분류하고 있습니다.\n분류는 내일 완료될 예정이며 이후에 2월 워크로그는 입력/수정이 불가능하니 참고 부탁드립니다.'
         }
    }
    r = requests.post(url, data=json.dumps(body), headers=headers)
