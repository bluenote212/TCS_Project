...
for i in range(0, len(pageid)):
    ...
    if soup.find('table'):
        ...
        for j in range(0,len(table)):
            table_td = table[j].find_all('td', limit=13)
...
data_reindex = data_drop.reindex(['test_date', 'SDK_Name(Device)', 'SDK_Name(Platform)', 'SDK_Name(Application)', 'Subtitle', 'Version', 'Test_Type', 'Module_Name',\
                                  'Module_Version', 'Pass', 'Fail', 'N/A', 'N/T', 'Total', 'Requirement_Cnt', 'Requirement_Coverage(%)', 'Codesonar_결함수',\
...
<p><br /></p><table><colgroup><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /></colgroup>\
...
for i in range(0, len(test_result_list)):
    ...
    for j in range(0, len(test_result_list[i])-1):
        if j==0:
            data_row += '<td><a href="' + test_result_list[i][19] + '">' + test_result_list[i][j] + '</a></td>'
        else:
