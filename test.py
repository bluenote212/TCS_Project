...
for i in range(0, len(pageid)):
    ...
    if soup.find('table'):
        ...
        for j in range(0,len(table)):
            table_td = table[j].find_all('td', limit=18) #테이블행에서 15개의 값 까지만 저장
...
data = pd.DataFrame(test_result, columns = ['test_date','author','reference&revision', 'SDK_Name(Device)', 'SDK_Name(Platform)', 'SDK_Name(Application)', 'Subtitle',\
                                            'Version', 'Test_Type', 'Module_Name', 'Module_Version', 'Pass', 'Fail', 'N/A', 'N/T', 'Total', 'Requirement_Cnt',\
                                            'Requirement_Coverage(%)', 'Codesonar_결함개수', 'QAC_결함개수', '전체코드라인수', 'URL'])

data_drop = data.drop(['author','reference&revision'], axis=1) #'author','reference&revision' 열 제거

data_reindex = data_drop.reindex(['test_date', 'SDK_Name(Device)', 'SDK_Name(Platform)', 'SDK_Name(Application)', 'Subtitle', 'Version', 'Test_Type', 'Module_Name',\
                                  'Module_Version', 'Pass', 'Fail', 'N/A', 'N/T', 'Total', 'Requirement_Cnt', 'Requirement_Coverage(%)', 'Codesonar_결함개수',\
                                  'QAC_결함개수', '전체코드라인수', 'URL'], axis=1)
    
data = data_reindex.sort_values('test_date')
data_dup = data.drop_duplicates(['SDK_Name(Device)', 'SDK_Name(Platform)', 'SDK_Name(Application)', 'Subtitle', 'Version','Module_Name', 'Module_Version'], keep='last')

#DB에 data를 저장
con = sqlite3.connect('C:/Users/B180093/database/tcs.db')
data_dup.to_sql('Unit_test2', con, if_exists='replace', index = False)
con.close()

#DB에서 값을 불러옴
con = sqlite3.connect('C:/Users/B180093/database/tcs.db')
test_result = pd.read_sql("SELECT * FROM Unit_test2", con)
con.close()
...
for i in range(0, len(test_result_list)):
    if test_result_list[i][4] != '':
        sdk_name = test_result_list[i][1] + '_' + test_result_list[i][2] + '_' + test_result_list[i][3] + '_' + test_result_list[i][4]
    else:
        sdk_name = test_result_list[i][1] + '_' + test_result_list[i][2] + '_' + test_result_list[i][3]
    test_result_list[i][1] = sdk_name
    del test_result_list[i][2:5]
...
<p><br /></p><table><colgroup><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /></colgroup>\
<tbody><tr><th>test_date</th><th>SDK_Name</th><th>Version</th><th>Test_Type</th>\
<th>Module_Name</th><th>Module_Version</th><th>Pass</th><th>Fail</th><th>N/A</th><th>N/T</th><th>Total</th><th>Req_Cnt</th><th>Req_Coverage(%)</th>\
<th>Codesonar_결함개수</th><th>QAC_결함개수</th><th>전체코드라인수</th></tr>'
...
for i in range(0, len(test_result_list)):
    ...
    for j in range(0, len(test_result_list[i])-1):
        if j==0:
            data_row += '<td><a href="' + test_result_list[i][16] + '">' + test_result_list[i][j] + '</a></td>'
        else:
