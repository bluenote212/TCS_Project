...
try:
    ...
    for i in range(0, len(test_result)):
        ...
        if 'Version' in test_result[i].keys():
            ...
            row_text.append('')
        if 'Test Chip' in test_result[i].keys():
            row_text.append(test_result[i]['Test Chip'])
        else:
            row_text.append('')
    ...
    data = pd.DataFrame(test_result_final, columns = ['test_date','author','reference&revision', 'SDK_Name_Device', 'SDK_Name_Platform', 'SDK_Name_Application', 'Subtitle',\
                                                'Version','Test Chip' , 'Test_Type', 'Module_Name', 'Module_subtitle', 'Module_Version', 'Pass', 'Fail', 'N/A', 'N/T', 'Total', 'Requirement_Cnt',\
    ...
    data_reindex = data_drop.reindex(['test_date', 'SDK_Name_Device', 'SDK_Name_Platform', 'SDK_Name_Application', 'Subtitle', 'Version','Test Chip' , 'Module_Name'\
                                     , 'Module_subtitle', 'Module_Version', 'Pass', 'Fail', 'N/A', 'N/T', 'Total', 'Codesonar_결함수', '수정불가_Codesonar_결함수',\
                                     'QAC_결함수', '수정불가_QAC_결함수', '전체코드라인수', 'URL'], axis=1)
    ...
    for i in range(0, len(data)):
        final.append({columns[0]:data[i][0], columns[1]:data[i][1], columns[2]:data[i][2], columns[3]:data[i][3], columns[4]:data[i][4], columns[5]:data[i][5], columns[6]:data[i][6]\
                     ...
                     , columns[19]:data[i][19],columns[20]:data[i][20]})
    ...
    <p><br /></p><table><colgroup><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /><col /></colgroup>\
    <tbody><tr><th>test_date</th><th>SDK_Name</th><th>Subtitle</th><th>Version</th><th>Test Chip</th>\
    ...
    body = {
        ...
        'content': {
            'type': 'text',
            'text': 'Wikicreate_unit_test.py 실행 실패했습니다.'
         }
    }
