...
for i in range(0, len(project_key)):
    ...
    if len(version) != 0:
        for j in range(0, len(version)):
            ...
            #duedate가 지난 이슈 개수를구하는 erquest
            url5 = requests.get('https://tcs.telechips.com:8443/rest/api/2/search?jql=duedate<now()%20and%20status%20not%20in(Resolved%2CClosed)%20and%20fixVersion%3D'+ version[j]['id'] +'&maxResults=1&fields=1', id_pw)
            duedate_over = json.loads(url5.text)
