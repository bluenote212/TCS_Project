...
for i in range(0, len(pageid)):
    ...
    if soup.find('table'):
        ...
        for j in range(0,len(table)):
            table_td = table[j].find_all('td', limit=12) #테이블행에서 14개의 값 까지만 저장
...
data = pd.DataFrame(test_result, columns = ['test_date', 'author', 'reference&revision', 'Solution(SDK)', 'Solution_Version', 'Platform', 'Chip_Set',\
                                            'Test_Type', 'Pass', 'Fail', 'N/A', 'N/T', 'Total', 'Requirement_Cnt', 'Requirement_Coverage(%)', 'URL'])

data_drop = data.drop(['author','reference&revision','Platform'], axis=1) #'author','reference&revision' 열 제거

data_reindex = data_drop.reindex(['test_date', 'Solution(SDK)', 'Solution_Version', 'Chip_Set', 'Test_Type', 'Pass', 'Fail', 'N/A', 'N/T', 'Total',\
...
<ac:parameter ac:name="name">SIT_test_backdata</ac:parameter><ac:rich-text-body>\
<p><br /></p><table><colgroup><col /><col /><col /><col /><col /><col /><col />\
<col /><col /><col /><col /><col /></colgroup><tbody><tr><th>test_date</th><th>Solution(SDK)</th><th>Solution_Version</th><th>Chip_Set</th><th>Test_Type</th>\
<th>Pass</th><th>Fail</th><th>N/A</th><th>N/T</th><th>Total</th><th>Requirement_Cnt</th><th>Requirement_Coverage(%)</th></tr>'

wiki_data_middle = ''

wiki_data_bottom = '</tbody></table><p class="auto-cursor-target"><br /><br /></p></ac:rich-text-body></ac:structured-macro><p class="auto-cursor-target"><br /></p>'
